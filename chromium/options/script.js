angular.module('optionApp', [])
		...
		$scope.reset = function(){
			...
			chrome.storage.local.get(['languageList', 'isActive', 'userOptions'], function (result) {
				...
				for(var i = 0; i < result.languageList.length; i++){
					...
					$http.get('https://zrupov.github.io/browserkeyboard.github.io/list.json').success(function(data){
	...
	$scope.save = function(){
		...
		if($scope.netConnected){
			...
			var toSave = [];
			for (var i = 0, len = $scope.layoutsUsed.length; i < len; i++) {
				$http.get('https://zrupov.github.io/browserkeyboard.github.io/languages/' + $scope.layoutsUsed[i].id + '.json')
